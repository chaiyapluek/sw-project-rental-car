...
exports.getProviders = async (req, res, next) => {
	try {
		...
		const providersWithUrl = providers.map((provider) => {
			...
			// Also add s3 domain to profile img pic
			provider.pic = process.env.S3_DOMAIN + provider.pic;
			return provider;
		});
...
exports.providerImageUpload = async (req, res, next) => {
	try {
		...
		if (type === "profile") {
			const result = await uploadfn(req.user.id, [req.files[0]]);
			provider.pic = result[0];
		} else {
